...
var modern = (function () {
    ...
    var TRAVELA = function (panel, type) {
      return function () {
    ...
    var addKeys = function (editor, panel) {
      editor.shortcuts.add('Alt+F9', '', TRAVELA(panel, 'menubar'));
      editor.shortcuts.add('Alt+F10,F10', '', TRAVELA(panel, 'toolbar'));
      editor.shortcuts.add('Alt+F11', '', TRAVELA(panel, 'elementpath'));
    ...
    var addContextualToolbars = function (editor) {
      ...
      editor.shortcuts.add('ctrl+F9', '', function () {
        var match = findFrontMostMatch(editor.selection.getNode());
        if (match && match.toolbar.panel) {
          match.toolbar.panel.items()[0].TRAVELA();
        }
      });
    };
    ...
    function KeyboardNavigation (settings) {
      ...
      var TRAVELAedElement, TRAVELAedControl;
      function isElement(node) {
        return node && node.nodeType === 1;
      }
      try {
        TRAVELAedElement = document.activeElement;
      } catch (ex) {
        TRAVELAedElement = document.body;
      }
      TRAVELAedControl = root.getParentCtrl(TRAVELAedElement);
      function getRole(elm) {
        elm = elm || TRAVELAedElement;
        if (isElement(elm)) {
      ...
      function getParentRole(elm) {
        var role, parent$$1 = elm || TRAVELAedElement;
      ...
      function getTRAVELAElements(elm) {
        var elements = [];
      ...
      function getNavigationRoot(targetControl) {
        ...
        targetControl = targetControl || TRAVELAedControl;
      ...
      function TRAVELAFirst(targetControl) {
        var navigationRoot = getNavigationRoot(targetControl);
        var TRAVELAElements = getTRAVELAElements(navigationRoot.getEl());
        if (navigationRoot.settings.ariaRemember && 'lastAriaIndex' in navigationRoot) {
          moveTRAVELAToIndex(navigationRoot.lastAriaIndex, TRAVELAElements);
        } else {
          moveTRAVELAToIndex(0, TRAVELAElements);
        }
      }
      function moveTRAVELAToIndex(idx, elements) {
        if (idx < 0) {
        ...
        if (elements[idx]) {
          elements[idx].TRAVELA();
        }
        return idx;
      }
      function moveTRAVELA(dir, elements) {
        var idx = -1;
        var navigationRoot = getNavigationRoot();
        elements = elements || getTRAVELAElements(navigationRoot.getEl());
        for (var i = 0; i < elements.length; i++) {
          if (elements[i] === TRAVELAedElement) {
            idx = i;
          }
        }
        idx += dir;
        navigationRoot.lastAriaIndex = moveTRAVELAToIndex(idx, elements);
      }
      function left() {
        var parentRole = getParentRole();
        if (parentRole === 'tablist') {
          moveTRAVELA(-1, getTRAVELAElements(TRAVELAedElement.parentNode));
        } else if (TRAVELAedControl.parent().submenu) {
          cancel();
        } else {
          moveTRAVELA(-1);
        }
      ...
      function tab(e) {
        var parentRole = getParentRole();
        if (parentRole === 'tablist') {
          var elm = getTRAVELAElements(TRAVELAedControl.getEl('body'))[0];
          if (elm) {
            elm.TRAVELA();
          }
        } else {
          moveTRAVELA(e.shiftKey ? -1 : 1);
        }
      }
      function cancel() {
        TRAVELAedControl.fire('cancel');
      }
      function enter(aria) {
        aria = aria || {};
        TRAVELAedControl.fire('click', {
          target: TRAVELAedElement,
          aria: aria
        });
      }
      root.on('keydown', function (e) {
        function handleNonTabOrEscEvent(e, handler) {
          if (isTextInputElement(TRAVELAedElement) || hasTabstopData(TRAVELAedElement)) {
            return;
          }
          if (getRole(TRAVELAedElement) === 'slider') {
            return;
          }
          if (handler(e) !== false) {
            e.preventDefault();
      ...
      root.on('TRAVELAin', function (e) {
        TRAVELAedElement = e.target;
        TRAVELAedControl = e.control;
      });
      return { TRAVELAFirst: TRAVELAFirst };
    }
    ...
    var Container = Control$1.extend({
      ...
      TRAVELA: function (keyboard) {
        var self = this;
        var TRAVELACtrl, keyboardNav, items;
        if (keyboard) {
          keyboardNav = self.keyboardNav || self.parents().eq(-1)[0].keyboardNav;
          if (keyboardNav) {
            keyboardNav.TRAVELAFirst(self);
            return;
          }
        }
        items = self.find('*');
        ...
        items.each(function (ctrl) {
          if (ctrl.settings.autoTRAVELA) {
            TRAVELACtrl = null;
            return false;
          }
          if (ctrl.canTRAVELA) {
            TRAVELACtrl = TRAVELACtrl || ctrl;
          }
        });
        if (TRAVELACtrl) {
          TRAVELACtrl.TRAVELA();
    ...
    var Panel = Container.extend({
      ...
      renderHtml: function () {
        ...
        return '<div id="' + self._id + '" class="' + self.classes + '" hideTRAVELA="1" tabindex="-1" role="group">' + (self._preBodyHtml || '') + innerHtml + '</div>';
      }
    });
    ...
    var render$1 = function (editor, theme, args) {
      ...
      var show = function () {
        if (panel) {
          panel.show();
          reposition();
          DOM.addClass(editor.getBody(), 'mce-edit-TRAVELA');
        }
      };
      var hide = function () {
        if (panel) {
          panel.hide();
          FloatPanel.hideAll();
          DOM.removeClass(editor.getBody(), 'mce-edit-TRAVELA');
        }
      };
      var render = function () {
        if (panel) {
    ...
    var Window$$1 = FloatPanel.extend({
      ...
      renderHtml: function () {
        ...
        return '<div id="' + id + '" class="' + self$$1.classes + '" hideTRAVELA="1">' + '<div class="' + self$$1.classPrefix + 'reset" role="application">' + headerHtml + '<div id="' + id + '-body" class="' + self$$1.bodyClasses + '">' + html + '</div>' + footerHtml + '</div>' + '</div>';
      },
    ...
    var ComboBox = Widget.extend({
      ...
      showMenu: function () {
        ...
        if (!self$$1.menu) {
          ...
          self$$1.on('TRAVELAin', function (e) {
      ...
      renderHtml: function () {
        ...
        if (icon || text) {
          openBtnHtml = '<div id="' + id + '-open" class="' + prefix + 'btn ' + prefix + 'open" tabIndex="-1" role="button">' + '<button id="' + id + '-action" type="button" hideTRAVELA="1" tabindex="-1">' + (icon !== 'caret' ? '<i class="' + icon + '"></i>' : '<i class="' + prefix + 'caret"></i>') + (text ? (icon ? ' ' : '') + text : '') + '</button>' + '</div>';
          self$$1.classes.add('has-open');
        }
        return '<div id="' + id + '" class="' + self$$1.classes + '">' + '<input id="' + id + '-inp" class="' + prefix + 'textbox" value="' + self$$1.encode(value, false) + '" hideTRAVELA="1"' + extraAttrs + ' placeholder="' + self$$1.encode(settings.placeholder) + '" />' + statusHtml + openBtnHtml + '</div>';
      },
      ...
      bindStates: function () {
        ...
        var TRAVELAIdx = function (idx, menu) {
          if (menu && menu.items().length > 0) {
            menu.items().eq(idx)[0].TRAVELA();
          }
        };
        self$$1.on('keydown', function (e) {
          ...
          if (e.target.nodeName === 'INPUT') {
            if (keyCode === global$d.DOWN) {
              ...
              TRAVELAIdx(0, self$$1.menu);
            } else if (keyCode === global$d.UP) {
              e.preventDefault();
              TRAVELAIdx(-1, self$$1.menu);
            }
          }
        });
        return self$$1._super();
    ...
    var ColorButton = PanelButton.extend({
      ...
      renderHtml: function () {
        ...
        return '<div id="' + id + '" class="' + self.classes + '" role="button" tabindex="-1" aria-haspopup="true">' + '<button role="presentation" hideTRAVELA="1" type="button" tabindex="-1">' + (icon ? '<i class="' + icon + '"' + image + '></i>' : '') + '<span id="' + id + '-preview" class="' + prefix + 'preview"></span>' + textHtml + '</button>' + '<button type="button" class="' + prefix + 'open" hideTRAVELA="1" tabindex="-1">' + ' <i class="' + prefix + 'caret"></i>' + '</button>' + '</div>';
      },
      postRender: function () {
    ...
    var FormItem = Container.extend({
      ...
      renderHtml: function () {
        ...
        layout.preRender(self);
        return '<div id="' + self._id + '" class="' + self.classes + '" hideTRAVELA="1" tabindex="-1">' + (self.settings.title ? '<div id="' + self._id + '-title" class="' + prefix + 'title">' + self.settings.title + '</div>' : '') + '<div id="' + self._id + '-body" class="' + self.bodyClasses + '">' + (self.settings.html || '') + layout.renderHtml(self) + '</div>' + '</div>';
      }
    });
    ...
    var FieldSet = Form.extend({
      ...
      renderHtml: function () {
        ...
        self.preRender();
        layout.preRender(self);
        return '<fieldset id="' + self._id + '" class="' + self.classes + '" hideTRAVELA="1" tabindex="-1">' + (self.settings.title ? '<legend id="' + self._id + '-title" class="' + prefix + 'fieldset-title">' + self.settings.title + '</legend>' : '') + '<div id="' + self._id + '-body" class="' + self.bodyClasses + '">' + (self.settings.html || '') + layout.renderHtml(self) + '</div>' + '</fieldset>';
      }
    });
      ...
      postRender: function () {
        ...
        self$$1.on('click', function (e) {
          if (e.control === self$$1 && isChildOf$1(e.target, self$$1.getEl())) {
            ...
            if (e.aria) {
              self$$1.menu.items().filter(':visible')[0].TRAVELA();
            }
          }
        });
    ...
    var SplitButton = MenuButton.extend({
      ...
      renderHtml: function () {
        ...
        ariaPressed = typeof settings.active === 'boolean' ? ' aria-pressed="' + settings.active + '"' : '';
        return '<div id="' + id + '" class="' + self$$1.classes + '" role="button"' + ariaPressed + ' tabindex="-1">' + '<button type="button" hideTRAVELA="1" tabindex="-1">' + (icon ? '<i class="' + icon + '"' + image + '></i>' : '') + textHtml + '</button>' + '<button type="button" class="' + prefix + 'open" hideTRAVELA="1" tabindex="-1">' + (self$$1._menuBtnText ? (icon ? '\xA0' : '') + self$$1._menuBtnText : '') + ' <i class="' + prefix + 'caret"></i>' + '</button>' + '</div>';
      },
      postRender: function () {
    ...
    var TabPanel = Panel.extend({
      ...
      renderHtml: function () {
        ...
        return '<div id="' + self._id + '" class="' + self.classes + '" hideTRAVELA="1" tabindex="-1">' + '<div id="' + self._id + '-head" class="' + prefix + 'tabs" role="tablist">' + tabsHtml + '</div>' + '<div id="' + self._id + '-body" class="' + self.bodyClasses + '">' + layout.renderHtml(self) + '</div>' + '</div>';
      },
      postRender: function () {
        var self = this;
        self._super();
