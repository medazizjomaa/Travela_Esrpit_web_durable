...
<html lang="en">
    ...
    <div class="impact-result">
        <p><strong>Transport choisi :</strong> <?= htmlspecialchars($transport) ?></p>
        <p><strong>Distance :</strong> <?= htmlspecialchars($distance) ?> km</p>
        <p><strong>Nombre de voyageurs :</strong> <?= htmlspecialchars($voyageurs) ?></p>
        <p><strong>Hébergement :</strong> <?= htmlspecialchars($hebergement) ?></p>
        <hr>
        <h3>Ton voyage génère environ 
            <span class="impact-co2"><?= round($co2_total, 2) ?> kg de CO₂</span>
        </h3>
        <p>🌳 Il faudrait planter <strong><?= round($co2_total/20) ?> arbres</strong> pour compenser.</p>
        <p>🔥 C'est équivalent à environ <strong><?= round($co2_total*5) ?> km</strong> en voiture.</p>
    </div>
    <a href="../controllers/index.php?action=formulaire" class="impact-btn">🔄 Refaire un calcul</a>
    <a href="../controllers/index.php?action=historique" class="impact-btn">📜 Voir l'historique</a>
</div>
